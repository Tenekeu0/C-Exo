...
T saisirAvecMessage(const std::string& message) {
    ...
    while (true) {
        ...
        if (std::cin >> valeur) {
            ...
            std::cout << "Entrée invalide. Veuillez entrer une valeur valide." << std::endl;
            std::cin.clear();
...
std::string saisirSecret() {
    return saisirAvecMessage<std::string>("Entrez la proposition à deviner : ");
}
...
void affichage::afficherMenuJeu2Joueurs() {
    ...
    std::cout << "Entrez 2 pour Historique" << std::endl;
    std::cout << "Entrez 3 pour Terminer" << std::endl;
}
...
void affichage::afficherMenuJeu() {
...
void affichage::affichageJeu() {
    ...
    while (!jeu.getEtatPartie()) {
        // Générer une proposition si c'est le tour de l'ordinateur (jeu à 1 joueur)
        if (jeu.getJoueur() == 1) {
            jeu.propositionModif(jeu.GenererProposition());
        }
        else {
            // Saisir la combinaison secrète du joueur 2
            ...
            while (!couleurValide) {
                ...
                if (!couleurValide) {
                    std::cout << "Saisie invalide : la combinaison secrète doit contenir exactement "
                        << jeu.getPiont() << " caractères valides parmi " << jeu.getcouleur() << ". ";
                }
            }
        ...
        while (!combTrouve && indexCoup < jeu.getCoups() && !jeu.getEtatPartie()) {
            ...
            if (jeu.getJoueur() == 2) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    // Afficher l'historique des propositions
                    std::cout << "Votre historique de coups : " << std::endl;
                    for (int i = 0; i < liste.nombreElements();++i) {
                        std::cout << "votre proposition " << i + 1 << " " << liste.historique()[i] << std::endl;
                    }
                    break;
                case 3:
